...
SentinelConnector.prototype.connect = function (callback, eventEmitter) {
  ...
  function connectToNext() {
    ...
    if (_this.currentPoint === _this.sentinels.length) {
      ...
      var retryDelay = typeof _this.options.sentinelRetryStrategy === 'function' ? _this.options.sentinelRetryStrategy(++_this.retryAttempts) : null;

      var errorMsg = typeof retryDelay !== 'number' ? 'All sentinels are unreachable and retry is disabled.' : 'All sentinels are unreachable. Retrying from scratch after ' + retryDelay + 'ms';
    ...
    _this.resolve(endpoint, function (err, resolved) {
      ...
      if (resolved) {
        ...
        var errorMsg = err ? 'failed to connect to sentinel ' + endpointAddress + ' because ' + err.message : 'connected to sentinel ' + endpointAddress + ' successfully, but got an invalid reply: ' + resolved;
    ...
    if (Array.isArray(result)) {
      for (var i = 0; i < result.length; ++i) {
        ...
        if (flags.indexOf('disconnected') === -1 && sentinel.ip && sentinel.port) {
          var endpoint = {
            host: sentinel.ip,
            port: parseInt(sentinel.port, 10)
          };
...
SentinelConnector.prototype.subSwitchMasterMessageForAllSentinels = function () {
  var _this = this;
  debug(`begin psubscribe pmessage +switch-master to sentinels`);
  _this.switchMaster = null;
  _this.subSwitchMasterSentinels = _.map(_this.sentinels, sentinelSetting => {
    if (typeof Redis === 'undefined') {
      Redis = require('../redis');
    }
    var sentinel = new Redis(sentinelSetting);
    sentinel.psubscribe('+switch-master', function (error, count) {});
    sentinel.on('pmessage', function (channel, pattern, message) {
      debug(`${pattern} : ${message}`);
      if (_.startsWith(message, _this.options.name)) {
        if (!_this.switchMaster) {
          _this.switchMaster = true;
          _.each(_this.subSwitchMasterSentinels, function(subClient) {
            subClient.disconnect();
          });
          debug(`emit stream close event`);
          _this.stream.emit('close');
        }
      }
    });
    return sentinel;
...
SentinelConnector.prototype.resolveMaster = function (client, callback) {
  ...
  client.sentinel('get-master-addr-by-name', this.options.name, function (err, result) {
    ...
    _this.updateSentinels(client, function (err) {
      ...
      _this.subSwitchMasterMessageForAllSentinels();
      callback(null, Array.isArray(result) ? {
        host: result[0],
        port: result[1]
      } : null);
    });
...
SentinelConnector.prototype.resolveSlave = function (client, callback) {
  ...
  client.sentinel('slaves', this.options.name, function (err, result) {
    ...
    callback(null, selectedSlave ? {
      host: selectedSlave.ip,
